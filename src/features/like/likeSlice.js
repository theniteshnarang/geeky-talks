...
export const likeSlice = createSlice({
    ...
    extraReducers: {
        ...
        [removeFromLikes.fulfilled]: (state, action) => {
            const foundLikeIndex = state.likes.findIndex(
                (like) => like._id === action.payload.id
            )
            if (foundLikeIndex > -1) {
...
export const findLikeById = (state, likeId) =>
    state.likes.likes.some((like) => like._id === likeId)
